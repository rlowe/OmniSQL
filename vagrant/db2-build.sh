if [[ -e /etc/redhat-release ]]; then
  sudo yum -y install Percona-Server-client-55 Percona-Server-devel-55 Percona-Server-server-55 Percona-Server-shared-55
  sudo rm -rf /etc/my.cnf
  sudo cp /omnisql/vagrant/db2-my.cnf /etc/my.cnf
  sudo service mysql start
elif [[ -e /etc/debian_version ]]; then
  sudo cp /omnisql/vagrant/db2-my.cnf /etc/mysql/my.cnf
